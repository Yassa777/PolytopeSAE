...
def compute_purity_metrics(parent_codes: torch.Tensor,
                          child_codes: torch.Tensor,
                          parent_labels: torch.Tensor,
                          child_labels: torch.Tensor) -> Dict[str, float]:
    """
    ...
    for parent_idx in range(n_parents):
        for child_idx in range(child_codes.shape[2]):
            ...
            child_binary_labels = (child_labels == (parent_idx * child_codes.shape[2] + child_idx)).cpu().numpy()
    ...
    return {
        'parent_purity_mean': np.mean(parent_purity_scores) if parent_purity_scores else 0.0,
        'parent_purity_std': np.std(parent_purity_scores) if parent_purity_scores else 0.0,
        ...
        'child_purity_mean': np.mean(child_purity_scores) if child_purity_scores else 0.0,
...
def compute_leakage_metrics(parent_codes: torch.Tensor,
                           child_codes: torch.Tensor,
                           sparsity_threshold: float = 0.01) -> Dict[str, float]:
    """
    ...
    parent_leakage_per_sample = torch.sum(parent_active, dim=1).float()  # How many parents active per sample
    
    # Child leakage: cross-child activation within parent
    child_active = child_codes > sparsity_threshold  # [batch_size, n_parents, n_children]
    child_leakage_per_parent = torch.sum(child_active, dim=2).float()  # [batch_size, n_parents]
    ...
    return {
        ...
        'child_leakage_mean': torch.mean(child_leakage_masked[child_leakage_masked > 0]).item() if torch.sum(child_leakage_masked > 0) > 0 else 0.0,
        'child_leakage_std': torch.std(child_leakage_masked[child_leakage_masked > 0]).item() if torch.sum(child_leakage_masked > 0) > 0 else 0.0,
        'fraction_multi_parent': torch.mean((parent_leakage_per_sample > 1).float()).item(),
        'fraction_multi_child': torch.mean((child_leakage_masked > 1).float()).item(),
    }


def compute_split_absorb_rates(child_codes: torch.Tensor,
                              parent_codes: torch.Tensor,
                              usage_threshold: float = 0.001) -> Dict[str, float]:
    """
    ...
    for parent_idx in range(n_parents):
        ...
        for child_idx in range(n_children):
            if children_usage[child_idx] > usage_threshold:
                ...
                usage_ratio = children_usage[child_idx] / (parent_usage[parent_idx] + 1e-8)
...
def compute_steering_leakage(original_logits: torch.Tensor,
                           steered_logits: torch.Tensor,
                           target_tokens: List[int],
                           non_target_tokens: Optional[List[int]] = None) -> Dict[str, float]:
    """
...
def compute_comprehensive_metrics(model,
                                data_loader,
...
def log_metrics_summary(metrics: Dict[str, float], 
    ...
    reconstruction_metrics = {k: v for k, v in metrics.items() 
                            if any(x in k.lower() for x in ['ev', 'ce', 'recon', 'loss'])}
    sparsity_metrics = {k: v for k, v in metrics.items() 
                       if any(x in k.lower() for x in ['sparsity', 'usage', 'active'])}
    purity_metrics = {k: v for k, v in metrics.items() 
                     if any(x in k.lower() for x in ['purity', 'auc', 'leakage'])}
    hierarchy_metrics = {k: v for k, v in metrics.items() 
