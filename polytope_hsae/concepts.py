    ...
    def select_children_for_parent(self, parent_synset_id: str) -> List[ConceptInfo]:
        ...
        for child_synset in children_synsets:
            ...
            score += (
                1
                if not any(
                    abstract_word in child_synset.definition().lower()
                    for abstract_word in ["abstract", "concept", "idea", "notion"]
                )
                else 0
            )
...
class PromptGenerator:
    ...
    def __init__(
        ...
        path = (
            Path(template_path)
            if template_path
            else Path(__file__).with_name("prompt_templates.json")
        )
    ...
    def generate_prompts_for_concept(
        self, concept: ConceptInfo, is_positive: bool = True
    ) -> List[str]:
        """
    ...
    def generate_contrast_prompts(
        self, target_concept: ConceptInfo, contrast_concepts: List[ConceptInfo]
    ) -> List[str]:
        """
        ...
        for contrast in contrast_concepts[:3]:  # Limit contrasts
            for template in contrast_templates:
                if "{verb}" in template:
                    ...
                    prompt = template.format(
                        target=target_concept.name, contrast=contrast.name
                    )
                    prompts.append(prompt)
    ...
    def populate_hierarchy_prompts(
        ...
        hierarchy.parent_prompts = self.generate_prompts_for_concept(
            hierarchy.parent, is_positive=True
        )
        ...
        for child in hierarchy.children:
            ...
            hierarchy.child_prompts[child.synset_id] = all_child_prompts[
                : self.prompts_per_concept
            ]
...
class ConceptSplitter:
    ...
    def split_hierarchies(
        self, hierarchies: List[ConceptHierarchy]
    ) -> Dict[str, List[ConceptHierarchy]]:
        """
        ...
        logger.info(
            f"Split {n_total} hierarchies: train={len(splits['train'])}, "
...
def load_concept_hierarchies(filepath: str) -> List[ConceptHierarchy]:
    ...
    for hierarchy_data in data:
        ...
        children = [
            ConceptInfo(**child_data) for child_data in hierarchy_data["children"]
        ]
