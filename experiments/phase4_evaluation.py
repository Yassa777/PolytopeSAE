...
from polytope_hsae.metrics import compute_comprehensive_metrics, log_metrics_summary
...
def load_models_and_data(config):
    ...
    teacher_model_file = base_dir / config['logging']['phase_3_log'] / "teacher_hsae_model.pt"
    baseline_model_file = base_dir / config['logging']['phase_2_log'] / "baseline_hsae_model.pt"
    ...
    if teacher_file.exists():
        ...
        parent_vectors = {k: torch.tensor(v) for k, v in teacher_data['parent_vectors'].items()}
        child_deltas = {
    ...
    concepts_file = base_dir / config['logging']['phase_1_log'] / "concept_hierarchies.json"
...
def compare_model_performance(teacher_metrics, baseline_metrics):
    ...
    metrics_to_compare = [
        '1-EV', '1-CE', 'parent_purity_mean', 'parent_leakage_mean',
        'child_leakage_mean', 'parent_sparsity', 'child_sparsity'
    ]
...
def run_steering_experiments(models, parent_vectors, child_deltas, hierarchies, config, exp_dir):
    ...
    for model_name, hsae_model in models.items():
        ...
        if test_prompts and parent_vectors:
            suite_results = steering.run_steering_suite(
                parent_vectors=dict(list(parent_vectors.items())[:5]),  # Limit for efficiency
                child_deltas={k: v for k, v in child_deltas.items() if k in list(parent_vectors.keys())[:5]},
...
def run_euclidean_vs_causal_ablation(parent_vectors, child_deltas, config, exp_dir):
    ...
    # Causal geometry
    causal_geometry = CausalGeometry(unembedding_matrix, shrinkage=config['geometry']['shrinkage'])
    ...
    for parent_id, deltas in child_deltas.items():
        ...
        for child_id, delta in deltas.items():
            ...
            # Causal angle
            causal_angle = torch.rad2deg(causal_geometry.causal_angle(parent_vector, delta))
    ...
    geometry_comparison = {
        ...
        'improvement': {
            'median_improvement': np.median(causal_angles) - np.median(euclidean_angles),
            'orthogonality_improvement': np.mean(causal_angles >= 80) - np.mean(euclidean_angles >= 80),
        }
    }
    
    logger.info(f"Euclidean median angle: {geometry_comparison['euclidean_angles']['median']:.1f}°")
    logger.info(f"Causal median angle: {geometry_comparison['causal_angles']['median']:.1f}°")
    logger.info(f"Improvement: {geometry_comparison['improvement']['median_improvement']:.1f}°")
...
def generate_final_report(ablation_results, steering_results, geometry_comparison, config, exp_dir):
    ...
    if 'teacher_vs_baseline' in ablation_results:
        ...
        summary['teacher_vs_baseline'] = {
            'purity_improvement': comparison.get('parent_purity_mean', {}).get('percent_improvement', 0),
            'leakage_reduction': comparison.get('parent_leakage_mean', {}).get('percent_improvement', 0),
            'reconstruction_maintained': comparison.get('success_criteria', {}).get('reconstruction_maintained', False),
            'overall_success': comparison.get('success_criteria', {}).get('overall_success', False),
        }
    ...
    if geometry_comparison:
        summary['geometry'] = {
            'causal_vs_euclidean_improvement': geometry_comparison.get('improvement', {}).get('median_improvement', 0),
            'causal_orthogonality_rate': geometry_comparison.get('causal_angles', {}).get('fraction_above_80', 0),
        }
    ...
    with open(summary_file, 'w') as f:
        ...
        if 'teacher_vs_baseline' in summary:
            ...
            f.write(f"Teacher vs Baseline:\n")
            f.write(f"  Purity improvement: {tvsb['purity_improvement']:.1f}%\n")
            f.write(f"  Leakage reduction: {tvsb['leakage_reduction']:.1f}%\n")
            f.write(f"  Reconstruction maintained: {tvsb['reconstruction_maintained']}\n")
            f.write(f"  Overall success: {tvsb['overall_success']}\n\n")
        
        if 'geometry' in summary:
            geom = summary['geometry']
            f.write(f"Geometry Analysis:\n")
            f.write(f"  Causal vs Euclidean improvement: {geom['causal_vs_euclidean_improvement']:.1f}°\n")
            f.write(f"  Causal orthogonality rate: {geom['causal_orthogonality_rate']:.3f}\n\n")
        
        if 'steering' in summary:
            steer = summary['steering']
            f.write(f"Steering Experiments:\n")
...
def main():
    ...
    success = True
    if 'teacher_vs_baseline' in ablation_results:
        success = ablation_results['teacher_vs_baseline'].get('success_criteria', {}).get('overall_success', False)
