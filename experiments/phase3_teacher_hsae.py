...
# Project imports
from polytope_hsae.models import HierarchicalSAE, HSAEConfig, create_teacher_initialized_hsae
...
def load_teacher_vectors(config):
    ...
    logger.info(f"Loaded {len(parent_vectors)} parent vectors and {sum(len(deltas) for deltas in child_deltas.values())} child deltas")
...
def load_activations(config, exp_dir):
    ...
    activations_file = Path(config['logging']['save_dir']) / config['logging']['phase_1_log'] / "activations.h5"
    ...
    activations = activation_capture.load_hierarchical_activations(str(activations_file))
...
def create_teacher_hsae(config, input_dim: int, parent_vectors, projectors, geometry):
    ...
    logger.info(f"Created teacher-initialized H-SAE with {sum(p.numel() for p in model.parameters())} parameters")
...
def compare_with_baseline(teacher_results, config):
    ...
    baseline_file = Path(config['logging']['save_dir']) / config['logging']['phase_2_log'] / "baseline_results.json"
    ...
    comparison['reconstruction'] = {
        ...
        'ev_difference': baseline_metrics.get('1-EV', 1.0) - teacher_metrics.get('1-EV', 1.0),
        'teacher_1_CE': teacher_metrics.get('1-CE', 10.0),
        'baseline_1_CE': baseline_metrics.get('1-CE', 10.0),
        'ce_difference': baseline_metrics.get('1-CE', 10.0) - teacher_metrics.get('1-CE', 10.0),
    }
    ...
    comparison['purity'] = {
        ...
        'purity_improvement': teacher_metrics.get('parent_purity_mean', 0.0) - baseline_metrics.get('parent_purity_mean', 0.0),
        'teacher_parent_auc': teacher_metrics.get('parent_auc_mean', 0.5),
        'baseline_parent_auc': baseline_metrics.get('parent_auc_mean', 0.5),
        'auc_improvement': teacher_metrics.get('parent_auc_mean', 0.5) - baseline_metrics.get('parent_auc_mean', 0.5),
    }
    ...
    comparison['leakage'] = {
        ...
        'leakage_reduction': (baseline_metrics.get('parent_leakage_mean', 10.0) - teacher_metrics.get('parent_leakage_mean', 10.0)) / baseline_metrics.get('parent_leakage_mean', 10.0),
        ...
        'child_leakage_reduction': (baseline_metrics.get('child_leakage_mean', 10.0) - teacher_metrics.get('child_leakage_mean', 10.0)) / baseline_metrics.get('child_leakage_mean', 10.0),
    }
    ...
    targets_met = {
        ...
        'reconstruction_parity': abs(comparison['reconstruction']['ev_difference']) <= 0.05,
    }
    ...
    logger.info("=" * 50)
    logger.info(f"Purity improvement: {comparison['purity']['purity_improvement']:.4f} (target: ≥0.10)")
    logger.info(f"Leakage reduction: {comparison['leakage']['leakage_reduction']:.4f} (target: ≥0.20)")
    logger.info(f"EV difference: {comparison['reconstruction']['ev_difference']:.4f} (target: ≤0.05)")
...
def main():
    parser = argparse.ArgumentParser(description="Phase 3: Teacher-Initialized H-SAE Training")
    ...
    if comparison:
        ...
        if success:
            ...
            logger.warning("⚠️  Teacher initialization shows mixed results - check targets")
